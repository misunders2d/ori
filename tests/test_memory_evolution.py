...
async def test_memory_crud():
    ...
    # 3. Update
    await memory.update("technical_test", f"id = '{rid}'", {"text": "Ori is a digital organism."})
    
    # 4. Search again
    results2 = await memory.search("technical_test", "Who is Ori?")
    assert len(results2) > 0
    # Search is semantic, so it should still find it.
    # Note: Search results are ranked by similarity.
    assert "organism" in results2[0]["text"]
    
    # 5. Delete
    await memory.forget("technical_test", f"id = '{rid}'")
