"""Phase-12 import-hygiene pins (carries the phase-11 slice-8
alias-robust detector verbatim).

Per ``docs/PHASE_12_PLAN.md`` §1.5 / §6 + the claude-reviewer
...
Phase-12 NEW modules (slice 1):
- ``app.v2.reasoning_enforcement`` — the pure
  read-only-reasoning composition layer.

``app.v2.validation`` / ``app.v2.runtime.worker`` are only
EXTENDED in later phase-12 slices (not new) — their hygiene is
...
@pytest.mark.parametrize("module_name", _PHASE12_MODULES)
...
def test_scope_collector_sees_a_real_module_scope_import():
    """reasoning_enforcement imports app.v2.tool_tags at
    module scope — assert the module-LOAD collector sees
    module-scope imports at all (forbid pin can't pass
    vacuously)."""
    imports = _module_scope_imports("app.v2.reasoning_enforcement")
    assert "app.v2.tool_tags" in imports
