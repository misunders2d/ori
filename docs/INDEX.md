...
## Tools (220 public functions across 39 files)
...
- `evolution_read_file` (line 56) — Reads the content of a file from the current agent's source code.
- `evolution_list_directory` (line 79) — Lists the files and folders inside a specific directory of the agent's source code.
- `evolution_stage_change` (line 124) — Stages a code change in a protected sandbox environment.
- `evolution_verify_sandbox` (line 181) — Runs verification checks on staged sandbox changes.
- `evolution_commit_and_push` (line 594) — Commits verified changes. Uses GitHub if configured, otherwise commits locally.
- `evolution_git_pull` (line 730) — Pulls the latest code from the GitHub remote repository into the current container and restarts.
- `evolution_git_reset` (line 762) — Resets the local workspace to match the last commit, deleting untracked 'dangling' files.
- `evolution_sync_local_to_upstream` (line 785) — Connects a detached local workspace to a remote GitHub repository and populates it.
- `evolution_git_fetch` (line 840) — Fetches latest refs from a remote without merging anything.
- `evolution_git_log` (line 864) — Shows compact commit history for a branch or ref.
- `evolution_git_diff_summary` (line 894) — Shows a compact summary of changes between two refs (files changed, insertions, deletions).
- `evolution_git_diff_file` (line 936) — Shows the actual diff for a specific file between two refs.
- `evolution_git_branches` (line 976) — Lists all local and remote branches with their latest commit.
...
- `update_self` (line 58) — Real process restart/reboot via supervisor exit signal.
- `trigger_rollback` (line 82) — Rollback code to previous commit and rebuild/restart.
- `session_refresh` (line 92) — Conversation/session reset only; wipes or summarizes chat history.
- `async set_planner_mode` (line 121) — Toggle deep thought.
- `async execute_approved_action` (line 126)
...
- **EvolutionToolset** — `app/toolsets/evolution.py:14`
  - Groups all self-evolution tools: read, stage, verify, commit, git ops.
  - tools: `check_installed_package`, `evolution_catalog`, `evolution_commit_and_push`, `evolution_git_branches`, `evolution_git_diff_file`, `evolution_git_diff_summary`, `evolution_git_fetch`, `evolution_git_log`, `evolution_git_pull`, `evolution_git_reset`, `evolution_import`, `evolution_list_directory`, `evolution_read_file`, `evolution_search`, `evolution_share`, `evolution_stage_change`, `evolution_sync_local_to_upstream`, `evolution_verify_sandbox`
...
- **SPApiToolset** — `app/toolsets/sp_api.py:5`
  ...
  - tools: `data_to_csv`, `export_report_to_csv`, `sp_check_report`, `sp_download_report`, `sp_get_account_health`, `sp_get_catalog_item`, `sp_get_competitive_pricing`, `sp_get_fees_estimate`, `sp_get_inventory_summaries`, `sp_get_listing`, `sp_get_order_items`, `sp_list_orders`, `sp_list_reports`, `sp_request_report`, `sp_search_catalog`

- **SystemToolset** — `app/toolsets/system.py:14`
  - Core system lifecycle tools.
  - tools: `execute_approved_action`, `list_spawned_agents`, `session_refresh`, `set_planner_mode`, `spawn_agent`, `stop_spawned_agent`, `trigger_rollback`, `update_self`
