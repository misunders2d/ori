...
## Coordinator mount + admin gate (slice 10)

`app/sub_agents/coordinator_agent.py` swaps the legacy single-tool
import `from app.tools.telegram import telegram_send_dm` for a mount
of the new `TelegramSkillsToolset()` (added to the `tools=[…]` list
right after `CreativesToolset()`). All twelve telegram tools are now
reachable on `CoordinatorAgent` via the toolset surface, not as an
individual function import.

`app/callbacks/guardrails/admin.py` adds the two capability-mutating
tools to the existing ACT+TOTP literal list at lines 91-104:
...
When the LLM invokes either tool, `admin_tool_guardrail` stages an
`ACT-XXXXXX` token (and a TOTP prompt when `REQUIRE_2FA=true`). Non-
admin callers are blocked with `Only Admin/Master users can invoke
…`. Read-only / self-scope telegram tools (`telegram_send_dm`,
`telegram_send_to_chat`, alias / cache list / resolve, `telegram_list_
capabilities`) are NOT in the gated list — they enforce their own
capability checks inline.

The deterministic poller-side `/cap grant` / `/cap revoke` commands
DO NOT flow through this guardrail. They mutate the same store via a
direct `capabilities.grant(...)` / `capabilities.revoke(...)` call
after an admin-env check, mirroring `/models set` (proposal §12 v2-a).
Both paths land in the same `data/capabilities.json` so the LLM-tool
path and the deterministic command path produce identical end state.

Tests (2 new files, 10 cases):
- `tests/test_telegram_admin_gate.py` (7) — parametrized over both
  tool names: admin staging without 2FA, admin staging with 2FA,
  non-admin blocked. Plus 1 test confirming the other nine telegram
  tools are NOT gated.
- `tests/test_coordinator_telegram_mount.py` (3) — module imports
  cleanly; mounted `TelegramSkillsToolset` returns the canonical 12
  tools; `from app.tools.telegram import telegram_send_dm` is no
  longer present in the coordinator source.

`docs/CALLBACKS.md` is fully auto-generated by `scripts/gen_docs.py`
("do not edit by hand"), so the gate change is documented in two
load-bearing places instead: the inline comment on the new entries in
`app/callbacks/guardrails/admin.py:91-104`, and this slice-10 section
of `docs/TELEGRAM_SKILLS.md`.

---

## (No remaining slices. Slice 11 wraps the final docs + auto-gen pass.)
