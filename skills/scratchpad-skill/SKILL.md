...
When a tool returns an oversized response (default >8000 chars, configurable via `TOOL_OUTPUT_SPILL_THRESHOLD`), the framework automatically writes the full output to a scratchpad named `_spill_<tool>_<hash>` and replaces the tool result with:
...
This is enforced by `tool_output_spillover_guardrail` in code; you don't trigger it manually. `_spill_*` pads are auto-cleaned at session end.
