...
_INSTRUCTION = (
    ...
    "- **Per-user Google connection** ('connect my Gmail/Calendar/Drive/"
    "Sheets', 'link my Google account', 'I want to authorize Google') → "
    "AmazonHeadAgent (which routes to AmazonWorkspaceAgent → `google_connect`). "
    "Do NOT call `configure_integration('google')` for per-user account "
    "linking — that tool is for admin-only multi-tenant OAuth client "
    "setup, not for regular users connecting their own accounts. "
    "`google_connect` is non-admin and works for everyone.\n"
    ...
    "After you create a plan, the workflow takes over and drives execution "
    "step by step automatically — you do NOT call `get_next_step` or "
    "`complete_step` (those tools no longer exist; the workflow handles "
    "step ordering and completion in code).\n"
    "- During plan execution you'll receive turns starting with "
    "`PLAN STEP N: <description>`. Execute exactly that step and reply "
    "with the result. Delegate to a sub-agent if appropriate; the result "
    "you return is recorded as the step's outcome.\n"
