...
async def spawn_agent(
    ...
    if model_overrides:
        for override in model_overrides.split(","):
            ...
            if "=" in override:
                ...
                model_env_flags.extend(["-e", f"MODEL_{component.strip().upper()}={model.strip()}"])
    ...
    run_cmd = [
        ...
        "-e", f"ADMIN_USER_IDS={merged_admin_ids}",
        "-e", f"AGENT_PURPOSE={purpose}",
        "-e", f"AGENT_RPM={child_rpm}",
