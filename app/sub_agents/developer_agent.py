...
developer_agent = Agent(
    ...
    instruction=(
        ...
        "7. COMMIT + REBOOT — Call `evolution_commit_and_push` ONLY if ALL checks pass. This is the ONLY admin approval in the cycle. "
        "After a successful commit, IMMEDIATELY request `update_self` from CoordinatorAgent — do NOT wait for another approval. "
        "The host supervisor will force-pull, rebuild, and restart. An evolution is NOT complete until reboot is triggered.\n\n"
        "ONE EVOLUTION = ONE COMMIT = ONE APPROVAL. Never split changes across multiple commits. "
        "Stage all files first, verify once, commit once, reboot once. If you need to change 5 files, "
        ...
        "commit-reboot cycle. If you skip the reboot, the running code diverges from the remote — "
