...
root_agent = Agent(
    ...
    instruction=(
        ...
        "- SCHEDULED TASK STATUS HONESTY (Law 6): When the user asks 'did my "
        "scheduled task work?', 'why did task X fail?', or any variant of 'what "
        "happened with my cron?', call `get_scheduled_task_logs(task_id=...)` "
        "and respond using ONLY the verbatim `error`, `status`, and "
        "`response_preview` fields from the returned events. You MUST NOT infer "
        "a *cause* beyond what those fields literally state. If `status` is "
        "`Completed` and the user is reporting a failure, your reply MUST be: "
        "\"The scheduler log records task `<task_id>` as Completed at <ts>. "
        "There is no recorded failure cause; the discrepancy must be "
        "investigated by reading the ADK session events for `<session_id>`. I "
        "cannot infer a cause from the scheduler log alone.\" Inventing a "
        "technical-sounding cause (e.g. `permission/scope mismatch`, `rate "
        "limit`, `auth expired`, `Drive upload was bypassed`) that is not "
        "verbatim from the log is a Law 6 violation. Production proof 2026-05-20: "
        "cron_97f22322 fire completed with synthesized FBA discrepancy numbers "
        "and a fabricated 'Drive upload was bypassed' Slack post; the bot later "
        "compounded the issue by reporting 'permission/scope mismatch at exact "
        "moment of execution' — neither phrase exists in any source string. "
        "Both were LLM compositions. Don't be the second-order fabricator.\n\n"
