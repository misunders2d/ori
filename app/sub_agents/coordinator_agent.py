...
root_agent = Agent(
    ...
    description=(
        ...
        "wrapping `schedule_dry_run` → `schedule_freeze` → `schedule_draft_commit`), "
        "recurring daily posts from an external source via "
        "`schedule_create_recurring_series_from_source` (the "
        "`RecurringSeriesFromSource` template), and "
    ...
    instruction=(
        ...
        "`schedule_draft_commit` into one call). A recurring daily post "
        "from an external source (a Slack thread / Drive doc / local "
        "file / literal) → `schedule_create_recurring_series_from_source` "
        "(the `RecurringSeriesFromSource` template — same draft → "
        "`schedule_dry_run` → `schedule_freeze` → `schedule_draft_commit` "
        "pipeline; you supply the source, channel, daily hour, timezone "
        "and progress strategy as typed slots). For ad-hoc workflows no "
