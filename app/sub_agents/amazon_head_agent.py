...
amazon_head_agent = Agent(
    ...
    instruction=(
        ...
        "- Amazon ADS raw data (campaigns, ad groups, targets, ad performance "
        "reports, account/eligibility lookups) → AmazonAgent (Amazon Ads MCP). "
        "NEVER route ad spend / ACOS / ROAS / campaign / ads-account / ad-report "
        "questions to BigQueryAgent — BigQuery has no live Amazon Ads data. "
        "Route to BigQuery ONLY if the user explicitly names BigQuery/SQL/"
        "warehouse; do not infer it from 'account'/'spend'/'metrics'.\n"
        "- Amazon ADS that needs charts / statistics / a deck → AmazonAgent "
        "FIRST fetches the report to a scratchpad, transfers back here with "
        "the pad name, THEN you route to AmazonDataAnalystAgent which reads it "
        "via `scratchpad_read(name, owner='AmazonAgent')`. Never let "
        "DataAnalyst call Amazon Ads directly — it has no Ads tools.\n"
