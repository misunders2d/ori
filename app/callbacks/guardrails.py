...
def file_attachment_capture(tool, args, tool_context, tool_response):
    ...
    try:
        state_dict = tool_context.state.to_dict() if tool_context else {}
    except Exception:
        state_dict = {}
    pending = list(state_dict.get(_PENDING_FILE_PARTS_KEY, []) or [])
...
def file_attachment_inject(callback_context, llm_response):
    ...
    """
    try:
        state_dict = callback_context.state.to_dict() if callback_context else {}
    except Exception:
        return None
