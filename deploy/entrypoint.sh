...
# Fix ownership
chown -R agentuser:agentgroup /code /home/agentuser 2>/dev/null || true
...
# Set ADC credentials path if copied by launcher.sh
ADC_LOCAL="/code/data/.adc.json"
if [ -f "$ADC_LOCAL" ] && [ -z "${GOOGLE_APPLICATION_CREDENTIALS:-}" ]; then
    export GOOGLE_APPLICATION_CREDENTIALS="$ADC_LOCAL"
    echo ":: [$BOT_NAME Container] ADC credentials loaded."
fi
